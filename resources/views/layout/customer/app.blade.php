        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarText">
                <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="#Layanan" aria-selected="true">Layanan</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="#TentangKami">Tentang Kami</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="#FAQ" aria-selected="true">FAQ</a>
                    </li>
                    @guest
                        @isset($login)
                        @else
                    ...
                    @else
                        <li class="nav-item my-auto mx-2">
                            <a class="btn btn-light shadow-sm rounded-pill py-1" href="{{ route('notifications') }}">
                                @if (auth()
            ->user()
            ->notifications->count() > 0)
                                    <span
                                        class="badge bg-danger float-end rounded-pill position-absolute m-2">{{ auth()->user()->notifications->count() }}</span>
                                @endif
                            ...
                            <div class="dropdown">
                                <a class="btn btn-light shadow-sm rounded-pill py-1" href="#" role="button"
                                    ...
                                    <img class="rounded-circle card-img"
                                        src="{{ asset(auth()->user()->avatar ?? 'src/bg.png') }}" alt=""
                                        style="height: 32px; width:32px; object-fit: cover;">
                                    <h6 href="" class="mb-0 align-middle d-inline">{{ auth()->user()->name }}</h6>
                                </a>
                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink">
                                    <li>
                                        <a href="{{ route('account.setting') }}" class="dropdown-item" type="button"><i
                                                class="fas fa-cog"></i> Setting</a>
                                    </li>
                                    <li>
                                        <a href="{{ route('order') }}" class="dropdown-item" type="button"><i
    ...
    <div class="modal fade" id="logoutModal" tabindex="-1" aria-labelledby="logoutModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('logout', ['web' => true]) }}" method="post">
                    @csrf
                    @method('post')
