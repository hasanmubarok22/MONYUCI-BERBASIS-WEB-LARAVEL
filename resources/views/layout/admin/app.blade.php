...
<body>
    <nav class="navbar fixed-top navbar-dark main-gradient-color navbar-expand-lg navbar-light bg-light">
        <div class="container-fluid">
            <a class="navbar-brand h1" href="{{ route('admin.index') }}">
            ...
            <div class="collapse navbar-collapse" id="navbarText">
                <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                    <li class="nav-item my-auto me-2">
                        <a class="btn btn-light shadow-sm rounded-pill py-1" href="{{ route('admin.notifications') }}">
                            @if (auth('admin')
        ->user()
        ->notifications->count() > 0)
                                <span
                                    class="badge bg-danger float-end rounded-pill position-absolute m-2">{{ auth('admin')->user()->notifications->count() }}</span>
                            @endif
                    ...
                    <li class="nav-item">
                        <div class="dropdown">
                            <a class="btn btn-light shadow-sm rounded-pill py-1" href="#" role="button" role="button"
                                id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false">
                                <img class="rounded-circle card-img"
                                    src="{{ asset(auth('admin')->user()->avatar ?? 'src/bg.png') }}" alt=""
                                    style="height: 32px; width:32px; object-fit: cover;">
                                <h6 href="" class="mb-0 align-middle d-inline">{{ auth('admin')->user()->name }}</h6>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink">
                                <li>
                                    <a href="{{ route('admin.setting') }}" class="dropdown-item" type="button"><i
