...
<body>
    <nav class="navbar fixed-top navbar-dark main-gradient-color navbar-expand-lg navbar-light bg-light">
        <div class="container-fluid">
            <a class="navbar-brand h1" href="{{ route('courier.index') }}">
                ...
                <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                    <li class="nav-item my-auto me-2">
                        <a class="btn btn-light shadow-sm rounded-pill py-1"
                            href="{{ route('courier.index', ['type' => 'notification']) }}">
                            @if (auth('courier')
        ->user()
        ->notifications->count() > 0)
                                <span
                                    class="badge bg-danger float-end rounded-pill position-absolute m-2">{{ auth('courier')->user()->notifications->count() }}</span>
                            @endif
                        ...
                        <div class="dropdown">
                            <a class="btn btn-light shadow-sm rounded-pill py-1" href="#" role="button"
                                ...
                                <img class="rounded-circle card-img"
                                    src="{{ asset(auth('courier')->user()->avatar ?? 'src/bg.png') }}" alt=""
                                    style="height: 32px; width:32px; object-fit: cover;">
                                <h6 href="" class="mb-0 align-middle d-inline">{{ auth('courier')->user()->name }}</h6>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink">
                                <li>
                                    <a href="{{ route('courier.setting') }}" class="dropdown-item" type="button"><i
                                            class="fas fa-cog"></i> Setting</a>
                                </li>
                                <li>
                                    <a href="{{ route('courier.index') }}" class="dropdown-item" type="button"><i
    ...
    <div class="modal fade" id="logoutModal" tabindex="-1" aria-labelledby="logoutModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('logout', ['courier' => true]) }}" method="post">
                    @csrf
                    @method('post')
